...
function hasUsefulData(customerName: string, rows: Row[], discount: string) {
  return Boolean(
    customerName.trim() ||
      discount.trim() ||
...
export default function Home() {
  ...
  const totalDays = calculatedRows.reduce((sum, row) => sum + row.days, 0);
  ...
  useEffect(() => {
    async function loadInitialDrafts() {
      try {
        ...
        if (
          currentDraft &&
          hasUsefulData(currentDraft.customerName, currentDraft.rows, currentDraft.discount) &&
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const usefulData = hasUsefulData(customerName, rows, discount);
        const now = Date.now();
  ...
  }, [customerName, discount, rows, loadedFromDb]);
...
ആകെ തുക: ₹${formatMoney(grandTotal)}${
      discountAmount > 0
        ? `\nഡിസ്‌കൗണ്ട്: ₹${formatMoney(
            discountAmount
          )}\nഡിസ്‌കൗണ്ടിന് ശേഷം: ₹${formatMoney(finalTotal)}`
        : ""
    }`;
  }
  ...
  <td>₹{formatMoney(grandTotal)}</td>
        ...
        <aside className="rightPanel">
          ...
          <div className="summaryGrid">
            ...
            <div className="summaryCard yellow">
              <span>💵 ആകെ തുക</span>
              <strong>₹{formatMoney(grandTotal)}</strong>
            </div>

            <div className="summaryCard purple">
              <span>📊 ദിവസ ശരാശരി</span>
              <strong>
                ₹{totalDays > 0 ? formatMoney(grandTotal / totalDays) : 0}
              </strong>
            </div>
          </div>
          ...
          {discountAmount > 0 ? (
            <>
              ...
              <div className="grandCard">
                <span>✅ ഡിസ്‌കൗണ്ടിന് ശേഷം</span>
                <strong>₹{formatMoney(finalTotal)}</strong>
              </div>
            </>
          ) : (
            <div className="grandCard">
              <span>🧾 ഗ്രാൻഡ് ടോട്ടൽ</span>
              <strong>₹{formatMoney(grandTotal)}</strong>
            </div>
          )}
      ...
      <div className="billCapture" id="professional-bill" ref={billRef}>
        ...
        <div className="billBottom">
          <div className="billSummary">
            <p className={discountAmount > 0 ? "" : "afterDiscount"}>
              <span>ആകെ തുക :</span>
          ...
          {discountAmount > 0 && (
            <div className="billFinal">
              <p>
                <span>ഡിസ്‌കൗണ്ട് :</span>
                <b>₹ {formatMoney(discountAmount)}</b>
              </p>
              <p className="afterDiscount">
                <span>ഡിസ്‌കൗണ്ടിന് ശേഷം :</span>
